...
public class FragmentPostDetails extends BaseFragment implements
    ...
    // 查询职位详情ID
//	private String kREQ_ID_findJobEntPostDetailDTO;
	...
	if (detailDTO.getIsApplied() == 0) { // 0 未申请
		...
		if (SystemServicesUtils.isCompleteInfo(getActivity())) {// 判断资料是否完善
			...
			switch (detailDTO.getPostStatus()) {
				...
				break;
			    case 2:
				PostApplyActivity_.intent(getActivity()).fairID(getActivity().getIntent().getExtras().getLong("fairID")).isFair(getActivity().getIntent().getExtras().getBoolean("isFair"))
	...
	if (tag == 1) { // 变为展开
	    ...
	    // skillsLayout.setLayoutParams(new
	    // LayoutParams(LayoutParams.WRAP_CONTENT,
	    // UnitTransfer.dip2px(getActivity(), 120) ));
	    v.setTag(0);
...
//					LinearLayout.LayoutParams.WRAP_CONTENT);
	    // skillsLayout.setLayoutParams(pa);;
	    v.setTag(1);
	...
	// 职位详情
	// jobPostService.findJobEntPostDetailDTO(ZcdhApplication.getInstance().getZcdh_uid(),
	// ((DetailsFrameActivity_)getActivity()).getPostId());

	// postIds.add(postId);
	jobPostService.findExtJobEntPostDetailDTO(
		getActivity().getIntent().getExtras().getLong("fairID"),
		ZcdhApplication.getInstance().getZcdh_uid(), postId).identify(kREQ_ID_findExtJobEntPostDetailDTO = RequestChannel.getChannelUniqueID(), this);
//		jobPostService
//				.findJobEntPostDetailDTO(
//						ZcdhApplication.getInstance().getZcdh_uid(), postId)
//				.identify(
//						kREQ_ID_findJobEntPostDetailDTO = RequestChannel
//								.getChannelUniqueID(),
//						this);
	...
	if (detailDTO != null) {
		...
		if (!StringUtils.isBlank(post.getJobCategory())) {
		    // jobCategoryText.setText(post.getJobCategory());
		    jobCategoryText
			    .setText(Html
				    .fromHtml("<b>类型</b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
					    + post.getJobCategory()));
		...
		if (!StringUtils.isBlank(post.getWorkExperience())) {
		    // jingyangText.setText(post.getWorkExperience());
		    jingyangText
			    .setText(Html
				    .fromHtml("<b>经验</b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
		...
		if (!StringUtils.isBlank(post.getDegree())) {
		    // degreeText.setText(String.format("学历:%s",
		    // post.getDegree()));
		    degreeText
			    .setText(Html
				    .fromHtml("<b>学历</b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
		...
		paymentText
			.setText(Html
				.fromHtml("<b>薪资</b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color='red'>"
					+ post.getSalary() + "</font>"));
		...
		// int match_progress = new
		// Double(post.getMatchRate()).intValue();
		// matchRatePb.setProgress(match_progress);
	...
	if (reqId.equals(kREQ_ID_findExtJobEntPostDetailDTO)) {
		...
		Toast.makeText(getActivity(), "无相关职位详情", Toast.LENGTH_SHORT)
			.show();
	    }
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
	    View itemView = LayoutInflater.from(getActivity()).inflate(
		    R.layout.post_other_item, parent,false);
	    TextView item = (TextView) itemView.findViewById(R.id.itemNameText);
...
//			showShare(false, null);
	...
	if (host.switchable()) {
		...
		Toast.makeText(getActivity(), "已经是最顶了", Toast.LENGTH_SHORT)
			.show();
	    } else {
		contentScroll.setVisibility(View.GONE);
		emptyTipView.startLoadingAnim();
		// host.finish();
		// ActivityDispatcher.toDetailsFrameActivity(getActivity(),
		// host.getPosts().get(host.getCurrentIndex() - 1).getPostId(),
		// true, host.getPosts(), host.getCurrentIndex() - 1);
	...
	DetailsFrameActivity host = (DetailsFrameActivity) getActivity();
	if (host.switchable()) {
	    if (host.getCurrentIndex() >= host.getPosts().size() - 1) {
		Toast.makeText(getActivity(), "已经是最后一项了", Toast.LENGTH_SHORT)
			.show();
	    } else {
		contentScroll.setVisibility(View.GONE);
		emptyTipView.startLoadingAnim();
	...
	switch (mode) {
		...
		if (host.switchable()) {
		    if (host.getCurrentIndex() > 0) {
			return host.getPosts().get(host.getCurrentIndex() - 1)
				.getPostAliases();
		    }
		}
		break;
