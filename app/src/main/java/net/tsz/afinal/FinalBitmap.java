...
public class FinalBitmap {
	...
	private PrepareBitmapListner mPrepareBitmapListner;
	...
	private FinalBitmap(Context context) {
		...
		String cachepath = Utils.getDiskCacheDir(context, "imgs").getAbsolutePath();
		Log.i("FinalBitmap", cachepath + "");
		configDiskCachePath(cachepath);//配置缓存路径
	...
	public FinalBitmap configAnimation(Animation animation){
		mConfig.defaultDisplayConfig.setAnimation(animation);
		return this;
	}
	
	public FinalBitmap configCaluSimple(boolean caluSimple){
		mConfig.defaultDisplayConfig.setCaluSimple(caluSimple);
		return this;
	}
	...
	/**
	 *  根据View 的rect 大小，在图片中截取部分
	 * @param imageView
	 * @param bitmap
	 */
	public void displayByPostScale(View imageView, Bitmap bitmap){
		
	}
	
	public void display(View imageView, String uri, PrepareBitmapListner prepareBitmapListner){
		this.mPrepareBitmapListner = prepareBitmapListner;
	...
	private void doDisplay(View imageView, String uri, BitmapDisplayConfig displayConfig) {
		...
		if (bitmap != null) {
			...
			/*Animation anima = mConfig.defaultDisplayConfig.getAnimation();
			if(anima!=null){
				imageView.startAnimation(anima);
			}*/
			if(mPrepareBitmapListner!=null){
				mPrepareBitmapListner.onPrepareDone();
			}
			...
			if (imageView instanceof ImageView) {
				if (imageView instanceof RoundedImageView) {
					((RoundedImageView) imageView).setImageDrawable(asyncDrawable);
				}
	...
	private Bitmap processBitmap(String uri, BitmapDisplayConfig config) {
		if (mBitmapProcess != null) {
			Log.i("getCaluSimple", config.getCaluSimple() + "");
