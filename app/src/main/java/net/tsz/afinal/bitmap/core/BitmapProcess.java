	...
	public Bitmap getBitmap(String url, BitmapDisplayConfig config) {
		...
		if(bitmap == null){
			...
			if(data != null && data.length > 0){
				if(config !=null)
					bitmap =  BitmapDecoder.decodeSampledBitmapFromByteArray(data,0,data.length,config.getBitmapWidth(),config.getBitmapHeight(), config.getCaluSimple());
				else
	...
	public Bitmap getFromDisk(String key,BitmapDisplayConfig config) {
        ...
        try {
            ...
            if ( found && buffer.length - buffer.offset > 0) {
    	        if( config != null){
    	            b = BitmapDecoder.decodeSampledBitmapFromByteArray(buffer.data,buffer.offset, buffer.length ,config.getBitmapWidth(),config.getBitmapHeight(),  config.getCaluSimple());
    	        }else{
