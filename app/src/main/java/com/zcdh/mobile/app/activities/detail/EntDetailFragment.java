...
public class EntDetailFragment extends BaseFragment implements
    ...
    private void initViews() {
        ...
        List<CommentDTO> commentDTOs = ent_detailDTO.getCommentDTO();
        ...
        ArrayList<HashMap<String, String>> imgStrings = new ArrayList<>();

        if (imgs != null) {
            for (ImgURLDTO imgModelDTO : imgs) {
                HashMap<String, String> hashImg = new HashMap<>();
...
//								.get(position).getOriginal() });
                if (imgs != null && imgs.size() > 0) {
                    ...
                    ActivityDispatcher.toPhotoBrowser(EntDetailFragment.this.getActivity(),
                            imgs.get(position).getOriginal(), urls);
                }
            }
        });
...
//			shareEnt();
    ...
    void onDial() {
        if (getUserId() != -1) {
            ...
            Toast.makeText(getActivity(),
                    getActivity().getResources().getString(R.string.login_first_then_call),
                    Toast.LENGTH_SHORT)
                    .show();
        }
    }
    ...
    void onMsgBtn() {
        if (ZcdhApplication.getInstance().getZcdh_uid() == -1) {
            Toast.makeText(getActivity(),
                    getActivity().getResources().getString(R.string.login_first_then_message),
                    Toast.LENGTH_SHORT).show();
            ActivityDispatcher.toLogin(getActivity());
        } else {
            if (SystemServicesUtils.isCompleteInfo(getActivity())) {
