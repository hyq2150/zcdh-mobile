...
//	private CustomDialog dialog;
    ...
    private ServiceHandler globalHandler = new ServiceHandler(this);

    private static class ServiceHandler extends Handler {

        private WeakReference<UpdateDataService> mReference;

        public ServiceHandler(UpdateDataService updateDataService) {
            mReference = new WeakReference<>(updateDataService);
        }

        @Override
        public void handleMessage(Message msg) {
            final UpdateDataService service = mReference.get();
            if (service != null) {
                switch (msg.what) {
                    case DO_NOTHING: {
                    ...
                    case DOWNLOADED_DATA: {
                        ...
                        SharedPreferencesUtil
                                .putValue(
                                        service.context,
                                        K.DB.kBASIC_DB_VERSION_KEY,
                                        service.updateInfo.getVersion());
                        service.dismissDialog();
                        Toast.makeText(service.context, "数据更新完成", Toast.LENGTH_SHORT).show();
                        //((WelcomeActivity)context).runBiz();
                        break;
                    }

                    case UPGRADE_DIALOG:
                        AlertDialog.Builder builder = new AlertDialog.Builder(service.context);
                        ...
                        builder.setNegativeButton("退出",
                                new DialogInterface.OnClickListener() {
                                    ...
                                    public void onClick(DialogInterface dialog, int id) {
                                        ...
                                        ((Activity) service.context).finish();
                                    }
                                });
