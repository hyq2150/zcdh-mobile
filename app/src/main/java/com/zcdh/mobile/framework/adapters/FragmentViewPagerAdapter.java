...
/**
 * 为ViewPager添加布局（Fragment），绑定和处理fragments和viewpager之间的逻辑关系 保存各个ViewPager 的state
 * ，确保来回切换重建UI Created with IntelliJ IDEA. Author: wangjie
 * email:tiantian.china.2@gmail.com Date: 13-10-11 Time: 下午3:03
 */
public class FragmentViewPagerAdapter extends PagerAdapter implements
	...
	public FragmentViewPagerAdapter(FragmentManager fragmentManager,
			ViewPager viewPager, ArrayList<Fragment> fragments,
			int defaultPager, OnExtraPageChangeListener listener) {
		this.fragments = fragments;
		this.fragmentManager = fragmentManager;
		this.viewPager = viewPager;
		this.viewPager.setAdapter(this);
		this.currentPageIndex = defaultPager;
		if (listener != null) {
			this.onExtraPageChangeListener = listener;
		} else {
			this.onExtraPageChangeListener = new OnExtraPageChangeListener() {
	...
	public Object instantiateItem(ViewGroup container, int position) {
		...
		if (fragment.getView().getParent() == null) {
			container.addView(fragment.getView(), 0); // 为viewpager增加布局
		}
