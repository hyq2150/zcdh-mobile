...
//                image.setImageBitmap(BitmapUtils.readBitMap(activity,R.drawable.mapguide_02));
                animaImageAction();
            }
            if (count == 1) {
                dismiss();
                SharedPreferencesUtil
                        .putValue(getContext(), Constants.kSHARE_NEW_USER_GUIDE_MAP, true);
        ...
        if (flag == Constants.flag_postlist) {
            dismiss();
            SharedPreferencesUtil
                    .putValue(getContext(), Constants.kSHARE_NEW_USER_GUIDE_LIST, true);
        }
    }
...
//            image.setImageBitmap(BitmapUtils.readBitMap(activity, R.drawable.mapguide_01));
        }
        if (flag == Constants.flag_postlist) {
            image.setImageResource(R.drawable.mapguide_03);
//            image.setImageBitmap(BitmapUtils.readBitMap(activity, R.drawable.mapguide_03));
        }
        animaImageAction();
        isShowing = true;
