...
public class OrdersFmUnPaid extends BaseFragment implements OnRefreshListener2<ListView>,
	...
	private static final String TAG = OrdersFmUnPaid.class.getSimpleName();
	...
	public String kREQ_ID_findMyOrderDTOByUnPaid;
	...
	public static OrdersFmUnPaid getInstance(int type) {
		OrdersFmUnPaid_ fm = new OrdersFmUnPaid_();
		...
		new AsyncTask<Void, Void, Page<MyOrderDTO>>() {
			...
			protected Page<MyOrderDTO> doInBackground(Void... params) {
				try {
					...
					// type, 1, page_size);
					service.findMyOrderDTOByUnPaid(ZcdhApplication.getInstance().getZcdh_uid(), 1, page_size).identify(kREQ_ID_findMyOrderDTOByUnPaid = RequestChannel.getChannelUniqueID(), OrdersFmUnPaid.this);
				} catch (Exception e) {
		...
		new AsyncTask<Void, Void, Page<MyOrderDTO>>() {
			...
			protected Page<MyOrderDTO> doInBackground(Void... params) {
				...
				try {
					...
					service.findMyOrderDTOByUnPaid(ZcdhApplication.getInstance().getZcdh_uid(), target_page, page_size).identify(kREQ_ID_findMyOrderDTOByUnPaid = RequestChannel.getChannelUniqueID(), OrdersFmUnPaid.this);
				} catch (Exception e) {
				}
				return page;
			}
	...
	public void onRequestSuccess(String reqId, Object result) {
		...
		if (reqId.equals(kREQ_ID_findMyOrderDTOByUnPaid)) {
			if (result != null) {
