...
/**
 * Created by fandy on 2015/9/2.
 */
public class TitlesAdapter extends BaseAdapter{
    private DisplayImageOptions options;

    private List<AdminAppConfigModelDTO> titles;

    private Context context;

    public void setTitles(List<AdminAppConfigModelDTO> titles) {
        this.titles = titles;
    ...
    public TitlesAdapter(Context ctx, List<AdminAppConfigModelDTO> titles) {
        this.titles = titles;
        context = ctx;
    ...
    public View getView(final int position, View convertView,
        ...
        if (position == titles.size()) {
            ...
            final AdminAppConfigModelDTO title = titles.get(position);
            holder.ll_app_item
                    .setBackgroundResource(R.drawable.white_corner_rect);
            holder.hotImg.setVisibility(View.VISIBLE);
            holder.tv_title.setVisibility(View.VISIBLE);
            if (title != null && title.getImgUrl() != null) {
                ImageLoader.getInstance()
                        .displayImage(title.getImgUrl(),
                                holder.image_view, options);
            }
            holder.tv_title.setText(title.getModel_name());
