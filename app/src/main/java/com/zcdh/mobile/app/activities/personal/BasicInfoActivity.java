...
public class BasicInfoActivity extends BaseActivity implements RequestListener, EditableDialogListener,
    ...
    @AfterViews
    void bindViews() {
        SystemServicesUtils.displayCustomTitle(this, getSupportActionBar(), "基本信息");
        SystemServicesUtils.displayCustomTitle(this, getSupportActionBar(),
                getString(R.string.activity_title_basic_info));
...
//            sendBroadcast(actionUpdateProfile);
    ...
    private void initView() {
        ...
        if (user_info_after.getBirth() != null)
            this.tvBirthday_content.setText(DateUtils.getDateByFormatYMD(user_info_after.getBirth()));
    ...
    private void notify_change() {
        changed = !ObjectUtils.compareJobUserInfo(user_info_after, user_info_original);
        supportInvalidateOptionsMenu();
    }
    ...
    @Background
    void doUpdate() {
        userService.updateJobUserInfoDTO(user_info_after)
                .identify(kREQ_ID_UPDATEJOBUSERINFODTO = RequestChannel.getChannelUniqueID(), this);
    ...
    void onLanguageBack(int resultCode, Intent data) {
        if (resultCode == RESULT_OK && data != null)
            tvLanguage_content.setText(String.valueOf(data.getIntExtra("language_count", 0)) + "种");
    }
